...
Список можно изменить в пункте «Музыкальные папки» на экране настроек.
Добавление открывает системный выбор каталога Windows, а удаление из списка
не удаляет сами файлы с диска. После изменения медиатека пересканируется сразу.

Начальный список также можно задать через `DWIJ_MUSIC_DIRS`.
...
После первого изменения через настройки сохранённый список становится основным;
`DWIJ_MUSIC_DIRS` снова используется, если удалить параметр
`desktop.music.directories` из `%APPDATA%\DWIJ\settings.properties`.
